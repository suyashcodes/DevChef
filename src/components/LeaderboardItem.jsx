...
const LeaderboardItem = ({ index, item, keys }) => {
    ...
    return (
        <motion.tr
            key={index}
            className={`${getRankingColor(index)} h-[50px]`}
        ...
        >{
                ...
                keys.map((key, index) => {
                    // if(!key.toLowerCase().startsWith('q'))
                        return <td key={index} className={` text-[13px] px-4 border-none ${index == 0 ? 'rounded-l-xl' : ''} ${index == keys.length - 1 ? 'rounded-r-xl' : ''}`}>{item[key]}</td>
                })
